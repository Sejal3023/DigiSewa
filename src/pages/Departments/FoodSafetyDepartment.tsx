...
import { 
  ...
  CheckCircle2, 
  XCircle,  
  Utensils, // ⚠️ CHANGE THIS ICON FOR EACH DEPARTMENT
  Send,
  AlertCircle 
} from 'lucide-react';
...
export default function FoodSafetyDepartment() { // ⚠️ CHANGE FUNCTION NAME
  // ========================================
  // ⚠️ CHANGE THESE 2 LINES FOR EACH DEPARTMENT
  // ========================================
  const DEPARTMENT_NAME = 'Food Safety Department';
  const LICENSE_TYPE = 'fssai-license';
  // ========================================
...
const handleApproveApplication = async (applicationId: string, remarks: string) => {
  console.log('═══════════════════════════════════════');
  console.log('🎯 APPROVAL STARTING (DATABASE MODE)');
  ...
  /* ❌ BLOCKCHAIN WALLET CONNECTION - COMMENTED OUT
  ...
  if (!walletConnected || !currentWalletAddress) {
    ...
    try {
      ...
      if (accounts.length === 0) {
        ...
        currentWalletAddress = newAccounts[0].address;
      } else {
      ...
      toast({
        ...
        description: error.message || "Please connect MetaMask manually",
        variant: "destructive"
      });
      return;
    }
  ...
  */
  ...
  try {
    ...
    console.log('📝 Step 1: Creating approval record in database...');
    ...
    console.log('✅ Approval record created:', approvalResponse.data);

    /* ❌ BLOCKCHAIN TRANSACTION - COMMENTED OUT
    // Step 3: Record approval on blockchain
    console.log('⛓️ Step 2: Recording approval on blockchain...');
    ...
    const approvalStamp = {
      ...
      action: 'APPROVED',
      officer: currentWalletAddress,
      timestamp: Date.now(),
      remarks: remarks || ''
    };
    
    const approvalHash = ethers.keccak256(
      ethers.toUtf8Bytes(JSON.stringify(approvalStamp))
    );
    ...
    const tx = await contract.verifyDocument(applicationId, approvalHash);
    console.log('⏳ Transaction sent, waiting for confirmation...');
    console.log('TX Hash:', tx.hash);

    toast({
      title: "Waiting for Confirmation...",
      description: "Transaction is being mined on blockchain",
    });
    ...
    */
    ...
    // ✅ Mock blockchain receipt for testing
    const receipt = { hash: 'TEST_TX_' + Date.now() };
    console.log('✅ Using test transaction hash:', receipt.hash);

    // Step 2: Update application with blockchain TX hash
    console.log('📝 Step 2: Updating application status...');
    await apiClient.patch(`/applications/${applicationId}`, {
      status: 'pending', // Keep as pending until super admin approval
      blockchain_tx_hash: receipt.hash,
      current_stage: 'department_approved'
    });
    console.log('✅ Application updated');

    /* ❌ APPROVAL UPDATE - COMMENTED OUT (optional)
    // Step 5: Update approval record with blockchain TX
    console.log('📝 Step 4: Updating approval record with blockchain TX...');
    await apiClient.patch(`/approvals/${approvalResponse.data.data.id}`, {
      blockchain_tx_hash: receipt.hash
    });
...
const handleRejectApplication = async (applicationId: string, remarks: string) => {
  console.log('═══════════════════════════════════════');
  console.log('🎯 REJECTION STARTING (DATABASE MODE)');
  console.log('Application ID:', applicationId);
  console.log('═══════════════════════════════════════');

  /* ❌ BLOCKCHAIN WALLET CHECK - COMMENTED OUT
  if (!walletConnected) {
    toast({
  ...
  */
  ...
  try {
    ...
    const rejectionStamp = `${applicationId}-${DEPARTMENT_NAME}-REJECTED-${Date.now()}-${remarks}`;
    const rejectionHash = ethers.keccak256(ethers.toUtf8Bytes(rejectionStamp));

    const tx = await contract.verifyDocument(applicationId, rejectionHash);

    toast({
      title: "Sending to Blockchain...",
      description: "Recording rejection",
    });
    ...
    */
    ...
    const receipt = { hash: 'TEST_REJECT_TX_' + Date.now() };
    console.log('✅ Using test transaction hash:', receipt.hash);

    // Step 1: Update application status
    console.log('📝 Step 1: Updating application to rejected...');
    await apiClient.patch(`/applications/${applicationId}`, {
      status: 'rejected',
      blockchain_tx_hash: receipt.hash,
    ...
    console.log('📝 Step 2: Creating rejection record...');
    ...
    */
    ...
    toast({
      ...
      description: error.response?.data?.message || error.message || "Failed to reject application.",
      variant: "destructive"
    });
  } finally {
    setProcessingAction(false);
