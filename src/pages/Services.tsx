...
const Services = () => {
  ...
  const [eligibilityStatus, setEligibilityStatus] = useState<Record<string, { canApply: boolean; reason?: string }>>({});
  ...
  // Fetch user's existing licenses and eligibility status
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Fetch user licenses
        const licenseResponse = await fetch("/api/licenses/user/certificates", {
          headers: {
        ...
        if (licenseResponse.ok) {
          ...
          if (data.success && data.data) {
            // Extract license types from user's certificates
            const licenseTypes = data.data.map((cert: any) => {
              // Map service names back to license types
              ...
              return serviceNameToType[cert.service_name] || cert.service_name;
            });
            setUserLicenses(licenseTypes);
          }
        }

        // Check eligibility for all services
        const eligibilityPromises = services.map(async (service) => {
          try {
            const response = await fetch(`/api/licenses/check-eligibility/${service.id}`, {
              headers: {
                Authorization: `Bearer ${token}`
              }
            });
            ...
            console.error(`Failed to check eligibility for ${service.id}:`, error);
            return {
              serviceId: service.id,
              canApply: true,
        ...
        const eligibilityMap: Record<string, { canApply: boolean; reason?: string }> = {};

        eligibilityResults.forEach(result => {
          eligibilityMap[result.serviceId] = {
            canApply: result.canApply,
            reason: result.reason
          };
        });
              ...
              >
                ...
                <CardContent>
                  ...
                  <div className="space-y-3">
                    ...
                    {/* License Status and Action Button */}
                    {userLicenses.includes(service.id) ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-center space-x-2 p-2 bg-green-50 rounded-md border border-green-200">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-sm font-medium text-green-800">License Already Issued</span>
                        </div>
                        <Button
                          className="w-full"
                          variant="outline"
                    ...
                    ) : eligibilityStatus[service.id] && !eligibilityStatus[service.id].canApply ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-center space-x-2 p-2 bg-red-50 rounded-md border border-red-200">
                          <AlertTriangle className="h-4 w-4 text-red-600" />
                          <span className="text-sm font-medium text-red-800">Not Eligible to Apply</span>
                        </div>
                        <p className="text-xs text-red-600 text-center">
                          {eligibilityStatus[service.id].reason}
                        </p>
                        <Button
                          className="w-full"
                          variant="outline"
